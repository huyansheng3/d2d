...
interface ParseInput {
  (config: InputConfig, form: { getFieldDecorator }): any;
}
    ...
    <Col {...colAttr} key={id}>
      <FormItem key={id} label={label} {...formItemLayout}>
        {getFieldDecorator(id, {
          ...
          <RadioGroup>{options.map(option => parseOption(option))}</RadioGroup>
        )}
      </FormItem>
    </Col>
  );
    ...
    <Col {...colAttr} key={id}>
      <FormItem key={id} label={label} {...formItemLayout}>
        {getFieldDecorator(id, {
          initialValue,
          rules,
        })(<Select>{options.map(option => parseOption(option))}</Select>)}
      </FormItem>
    </Col>
  );
};
...
export const parseCheckbox = (config, form) => {
  let {
    id,
    rules,
    label = '',
